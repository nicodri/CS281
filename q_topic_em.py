        ...
        for w in fileData[t]:
            if w not in posterior_w_z:
                ...
                for z in range(NUM_TOPICS):
                    # '''calculate the posterior probability posterior_w_z[w][z]
                    ...
                    sum_z += posterior_w_z[w][z]
                for z in range(NUM_TOPICS):
                    posterior_w_z[w][z] = posterior_w_z[w][z]/sum_z

            for z in range(NUM_TOPICS):
                '''Update soft counts count_t_z[t][z] '''
                count_t_z[t][z] += posterior_w_z[w][z]

                '''Update soft counts count_w_z[w][z] '''
...
def m_step(count_t_z, count_w_z):
    ...
    '''Get the max Likelihood estimate of theta_t_z[t][z]
		 Make sure that the theta_t_z[t] is a valid probability 
					distribution over the topics z.'''
    for t in range(np.shape(count_t_z)[0]):
        sum_t = sum(count_t_z[t, :])
        for z in range(np.shape(count_t_z)[1]):
            theta_t_z[t][z] = (count_t_z[t][z])/(sum_t)

    '''Get the max Likelihood estimate of theta_z_w[t][z]
	   Make sure that the theta_z_w[z] is a valid probability 
					distribution over the words.'''
    for z in range(np.shape(count_w_z)[1]):
        sum_z = sum(count_w_z[:, z])
        for w in range(np.shape(count_w_z)[0]):
            theta_z_w[z][w] = (count_w_z[w][z])/(sum_z)
...
if __name__ == '__main__':
    ...
    f = open("topics_"+str(NUM_TOPICS)+"_"+str(num_iter)+".txt", "w")
    ...
    for z in range(NUM_TOPICS):
        ...
        print "Topic", z+1
        f.write("Topic"+str(z+1)+"\n")
        for j in range(20):
            print wordProb[j][0], '(%.4f),' % wordProb[j][1],
            f.write(str(wordProb[j][0])+'(%.4f),' % wordProb[j][1])
        print '\n'
